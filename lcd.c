...
void draw_graph(void){
  LCD_DrawLine(0,0,0,LCD_H, WHITE); //Y axeln
  for (int i = 0; i < 9; i++)
  {
    LCD_DrawPoint(1,i*10, WHITE);
  }
  
  LCD_DrawLine(0,LCD_H/2,LCD_W,LCD_H/2, WHITE); //X axeln
  for (int i = 0; i < 17; i++)
  {
    LCD_DrawPoint(i*10,(LCD_H/2)+1, WHITE);
  }
}

void LCD_Draw128Points(int points[], int prevPoints[]){  
	
	for(int i = 127; i != 0; i--){                                     
		LCD_DrawPoint(i,prevPoints[i],BLACK);
	}
	for(int i = 127; i != 0; i--){                                     
		LCD_DrawPoint(i,points[i],WHITE);
	}
}
